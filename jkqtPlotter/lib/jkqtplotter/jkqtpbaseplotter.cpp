 ...
 * JKQTBasePlotter
 **************************************************************************************************************************/
JKQTBasePlotter::JKQTBasePlotter(bool datastore_internal, QObject* parent, JKQTPDatastore* datast, const QString& _exclusionString ):
    QObject(parent), plotterStyle(JKQTPGetSystemDefaultBaseStyle()), m_plotsModel(nullptr), xAxis(nullptr), yAxis(nullptr)
{
    ...
    mainKey=new JKQTPMainKey(this, _exclusionString);
...
void JKQTBasePlotter::zoomIn(double factor) {
    ...
    for (auto ax: getYAxes(true)) {
        ...
        const double xmax=ax->p2x(new_ma);
        // one of three places where we constrain 2D translation of the plotspace when in partial data scope/subplot mode...
        if (show_full_scope || (xmin>sub_plot_ymin && xmax<sub_plot_ymax))
            ax->setRange(qMin(xmin, xmax), qMax(xmin, xmax));
    }
...
void JKQTBasePlotter::setLegendExclusionText(QString legendExclusionText)
{
    legendExclusionString = legendExclusionText;
}
...
bool JKQTBasePlotter::exportpreview(QSizeF pageSize, bool unitIsMM) {
    ...
    QGridLayout* layout=new QGridLayout();
    dlg->setLayout(layout);
    dlg->setWindowTitle(tr("Graph clipboard preview ..."));
    dlg->setWindowIcon(QIcon(":/JKQTPlotter/jkqtp_exportprintpreview.png"));
...
bool JKQTBasePlotter::getGraphsYMinMax(double& miny, double& maxy, double& smallestGreaterZero, const PlotElementPreciate &predicate) {
    ...
    size_t count=0;
    for (int i=0; i<graphs.size(); i++) {
        if (graphs[i]->getTitle() != "Frame Line")
        {
            if (graphs[i]->isVisible() && predicate(graphs[i])) {
                double gminy=0;
                double gmaxy=0;
...
void JKQTBasePlotter::showPlotData() {
    ...
    // Remove Frameline columns.
    JKQTPDatastore *cleanedDS = getDatastore();
    for (int i = cleanedDS->getColumnCount(); i >0; --i)
    {
        QString title = cleanedDS->getColumnName(i-1);
        if (title.contains(legendExclusionString))
            cleanedDS->deleteColumn(i-1);
    }

    JKQTPDatastoreModel* model=new JKQTPDatastoreModel(cleanedDS, this);
    tv->setModel(model);
