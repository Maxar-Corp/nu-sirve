...
JKQTPlotter::JKQTPlotter(bool datastore_internal, QWidget* parent, JKQTPDatastore* datast, QString jkqptKeyFramelineTitle):
    ...
    actgrpMouseLeft(nullptr), actMouseLeftAsDefault(nullptr), actMouseLeftAsRuler(nullptr), actMouseMoveToolTip(nullptr), actMouseLeftAsZoomRect(nullptr), actMouseLeftAsPanView(nullptr)
{
    ...
    plotter=new JKQTBasePlotter(datastore_internal, this, datast, jkqptKeyFramelineTitle);
...
void JKQTPlotter::setExclusionString(QString exclusionString)
{
    this->plotter->setLegendExclusionText(exclusionString);
}
...
void JKQTPlotter::wheelEvent ( QWheelEvent * event ) {
    ...
    if (foundIT) {
        // this heuristic recognizes pan-gestures on a track-pad.
        ...
        if (forcePan || doPan) {
            ...
            if (abs(angleDelta.y())>30 && angleDelta.x()==0) {
                factor=pow(1.05, 1.0*static_cast<double>(angleDelta.y())/120.0);
            } else if (abs(angleDelta.x())>30 && angleDelta.y()==0) {
                factor=pow(1.05, 1.0*static_cast<double>(angleDelta.x())/120.0);
            }
        }
    }
    ...
    if (acTodo==WheelActionType::Zoom && factor!=1.0)
    {
        ...
        if (within_range_x && within_range_y)
            plotter->setXY(xmin, xmax, ymin, ymax, true);
    }
