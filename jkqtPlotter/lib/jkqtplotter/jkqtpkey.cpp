...
JKQTPBaseKey::JKQTPBaseKey(JKQTBasePlotter* _parent, QString _removalString):
    QObject(_parent),
    parent(_parent),
    localKeyStyle()
{
    framelineRemovalString = _removalString;
}
...
{
    for (auto& c: layout.d->columns) {
        for (int i = c.rows.size() - 1; i >= 0; --i) {
            if (c.rows[i].text.contains(framelineRemovalString, Qt::CaseInsensitive)) {
                c.rows.removeAt(i);  // Safe removal
            }
        }
    }
...
JKQTPMainKey::JKQTPMainKey(JKQTBasePlotter *parent, QString framelineRemovalString):
    JKQTPBaseKey(parent, framelineRemovalString)
{
