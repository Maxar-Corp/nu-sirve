...
ManualPlottingTrackDetails TrackInformation::calculate_az_el(int frame_number, int centroid_x, int centroid_y)
{
    ...
    std::vector<double> az_el_result = AzElCalculation::calculate(centroid_x, centroid_y, eng_data.boresight_lat, eng_data.boresight_long, eng_data.dcm, eng_data.i_fov_x, eng_data.i_fov_y, adjust_frame_ref, adjust_base, adjust_rounding_err_az, adjust_rounding_err_el );
