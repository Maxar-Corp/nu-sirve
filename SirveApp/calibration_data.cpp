...
bool CalibrationData::set_calibration_details(QString path_to_nuc, QString path_to_image, SelectedData selection1,
                                              SelectedData selection2, double int_time)
{
...
void CalibrationDialog::InitializeGui()
{
    ...
    QLabel* description = new QLabel("Select two temperature ranges to calibrate between. To select a temperature "
                                     "range, left click and drag across the chart");
...
void CalibrationDialog::UpdateUserSelectionLabels(SelectedData& data)
{
    ...
    if (data.valid_data)
    {
        radio_temperature->setText(QString("Temperature #%4: %1C, +/- %2 [Over %3 frames]")
                                       .arg(data.temperature_mean, 0, 'f', 2)
                                       .arg(data.temperature_std, 0, 'f', 3)
                                       .arg(data.num_frames)
                                       .arg(data.id));
    }
    else
