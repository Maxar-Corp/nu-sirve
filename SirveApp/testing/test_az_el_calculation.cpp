...
void TestAzElCalculation::test_azimuth_elevation_calculation()
{
    ...
    The indices of the human-user-interface viewport (the 640x480 gui window) start at 1 for the pinpoint feature.
    ...
    */
    ...
    // NOTE: Due to a situation where the OSM centroid double is rounded to an integer, the OSM track for this frame is slightly off the pinpoint pixel.
    // Adjusting this one to the left makes the math work for this frame.
	
    int adjusted_track_x = pinpoint_track_x - 2; // The number here 2 accounts for 1->0 indexed translation, plus the rounding error correction.
    int adjusted_track_y = pinpoint_track_y - 1; // This number accounts for 1->0 indexed translation only.
    int osm_x = 302;
    int osm_y = -35;
