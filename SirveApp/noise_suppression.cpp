...
std::vector<std::vector<uint16_t>> AdaptiveNoiseSuppression::process_frames_fast(int start_frame, int number_of_frames, video_details & original,  QString & hide_shadow_choice, QProgressDialog & progress)
{
	...
	if (hide_shadow_choice == "Hide Shadow"){
		for (int i = 0; i < num_video_frames; i++){
			...
			NoiseSuppressionGeneral::remove_shadow(frame_vector);
...
void NoiseSuppressionGeneral::remove_shadow(arma::vec & frame_vector)
{
	frame_vector = frame_vector - arma::mean(frame_vector);
	arma::colvec I = arma::ones(frame_vector.size());
	arma::uvec index_other = arma::find(arma::abs(frame_vector) <= 3. * arma::stddev(frame_vector));
	arma::uvec index_negative = arma::find(frame_vector < -2. * arma::stddev(frame_vector));
	if (index_negative.size() > 0) {				
		frame_vector.elem(index_negative) = arma::randn<arma::vec>(index_negative.size(),arma::distr_param(arma::mean(frame_vector.elem(index_other)),arma::stddev(frame_vector.elem(index_other))));
	}
	frame_vector -= frame_vector.min();
}