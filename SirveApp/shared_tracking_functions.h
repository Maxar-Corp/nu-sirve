...
namespace SharedTrackingFunctions
{
    ...
    void GetTrackPointData(std::string & trackFeature, int & threshold, cv::Mat & frame_crop, cv::Mat & raw_frame_bbox, cv::Mat & frame_bbox_threshold, cv::Point & frame_point, double & peak_counts, double & mean_counts, cv::Scalar & sum_counts, uint32_t & number_pixels);
